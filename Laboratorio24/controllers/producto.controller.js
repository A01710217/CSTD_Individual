...
exports.get_buscar = (request, response, next) => {
    Producto.search(request.params.valor_busqueda || '').then(([rows, fieldData]) => {
        return response.status(200).json({productos: rows});
    })
    .catch((error) => {
        console.log(error);
...
exports.post_agregar = (request, response, next) => {
    ...
    Producto.fetchAll().then(([rows, fieldData]) => {
        filesystem.writeFileSync('Laboratorio24/productos.txt', '');
        for (let i = 0; i < rows.length; i++) {
            filesystem.appendFileSync('Laboratorio24/productos.txt', rows[i].nombre  + "\n" + rows[i].precio + "\n" + rows[i].texto + "\n" + rows[i].url+ "\n\n");
        }
    })
    .catch((error) => {
        console.log(error);
